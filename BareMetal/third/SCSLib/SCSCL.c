...
int WritePos(uint8_t ID, uint16_t Position, uint16_t Time, uint16_t Speed) // 10ms
{
    if (Position > 2100)
        Position = 2100;
...
void SyncWritePos(uint8_t ID[], uint8_t IDN, uint16_t Position[], uint16_t Time[], uint16_t Speed[]) // 2000ms/1000 = 2ms
{
    uint8_t offbuf[32 * 6];
    uint8_t i;
    for (i = 0; i < IDN; i++) {
