...
static I2C_Device        s_MPU6050;
static LPF_TypeDef       s_lpf_roll;
...
{
    uint8_t id;
    if (HAL_OK == MPU6050_ReadRegister(&s_MPU6050, MPU6050_WHO_AM_I, &id, 1))
        return id;
    else
        return -1;
}
...
void MPU6050_ReadData(uint8_t *data) // 10字节380us
{
    ...
    if (data) {
        status |= MPU6050_ReadRegister(&s_MPU6050, MPU6050_ACCEL_XOUT_H, &data[0], 6);
        status |= MPU6050_ReadRegister(&s_MPU6050, MPU6050_GYRO_YOUT_H, &data[6], 4);
    }
...
{
    HAL_StatusTypeDef status = HAL_OK;
    Device_I2C_Register(&s_MPU6050, &hi2c3, MPU6050_ADDR, I2C_DEVICE_MPU6050);
    status |= MPU6050_WriteRegister(&s_MPU6050, MPU6050_PWR_MGMT_1, 0x00);
    status |= MPU6050_WriteRegister(&s_MPU6050, MPU6050_PWR_MGMT_2, 0x00);
    status |= MPU6050_WriteRegister(&s_MPU6050, MPU6050_SMPLRT_DIV, 0x09);
    status |= MPU6050_WriteRegister(&s_MPU6050, MPU6050_CONFIG, 0x06);
    status |= MPU6050_WriteRegister(&s_MPU6050, MPU6050_GYRO_CONFIG, 0x18);
    status |= MPU6050_WriteRegister(&s_MPU6050, MPU6050_ACCEL_CONFIG, 0x18);
    LPF_Init(&s_lpf_roll, 0, 0.2);
    if (status == HAL_OK) {
...
void MPU6050_ParseData(int16_t *data, float *Ang_roll, float *Ang_pitch)
{
    ...
    *Ang_roll = LowPassFilter(&s_lpf_roll, roll);
    *Ang_pitch = pitch;
}
